...
class BibFile():
	...
	def getInfo(self):
		...
		for pos, pub in enumerate(self.publications):
			...
			for posTag, tag in enumerate(self.bibType.bibTags): # For each tag for this type of publication
				xmlIndex = self.bibType.indexes[posTag] # Gets the xml index for this tag
				xmlKey = self.bibType.xmlKeys[posTag] # Gets the xml key for this tag
				...
				if tag == 'pages':
					...
					publicationInfo['Field']['Tag'].append(tag) # Adds the tag pages
					publicationInfo['Field']['Info'].append(pages) # Adds the pages

				elif tag == 'journal' or tag == 'booktitle' or tag == 'title': # If it's one of these tags the title case will be applied to it
					attrib = pub[xmlIndex].attrib[xmlKey]
					attrib = titleCase(attrib)
					publicationInfo['Field']['Tag'].append(tag) # Adds the tag
